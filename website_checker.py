...
def search_business_online(business_name, retries=3):
    query = f'"{business_name}" Nigeria official website'
...
def check_website(business_name):
    ...
    if not real_links:
        return {"has_website": False, "website_status": "No Website Found", "website_url": ""}
    top_link = real_links[0]
    if validate_website(top_link):
        return {"has_website": True, "website_status": "Has Website", "website_url": top_link}
    else:
        return {"has_website": False, "website_status": "Broken Website", "website_url": top_link}
